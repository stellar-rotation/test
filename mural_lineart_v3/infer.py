...
def extract(image_path, mask_path=None, ckpt=None, device=None, use_ema=True):
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    if ckpt is None:
        ema_path = str(config.CHECKPOINT_DIR / "unified_ema.pt")
        best_path = str(config.CHECKPOINT_DIR / "unified_best.pt")
        from pathlib import Path
        ckpt = ema_path if (use_ema and Path(ema_path).exists()) else best_path
    ...
    G.load_state_dict(c["model_G"]); G.eval()
    print(f"Loaded: {ckpt}")
