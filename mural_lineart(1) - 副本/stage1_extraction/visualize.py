...
def generate_visualization(image_path, mask_path=None, gt_path=None, checkpoint_path=None, output_path=None, device=None):
    ...
    if checkpoint_path is None:
        checkpoint_path = str(config.FINAL_CHECKPOINT)
    ...
    if gt_path:
        ...
        # 尝试根据 config 中定义的路径自动推断线稿 GT 路径
        possible_gt_paths = [
            str(config.TRAIN_EDGES / Path(image_path).name),
            str(config.VAL_EDGES / Path(image_path).name),
            str(config.TEST_EDGES / Path(image_path).name),
        ]
