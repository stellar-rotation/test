...
# ── 训练参数 ──
SEED              = 30         # 本实验负责人统一随机种子
DETERMINISTIC     = True       # 消融实验优先保证可复现性
...
BETA2             = 0.999
LR_DECAY_EPOCH    = 100
EARLY_STOP_PATIENCE = 20     # 验证指标连续 20 轮未改善则早停
...
# ── 损失消融实验预设 ──
# L1、Edge、GAN 始终保留；包含单项删除及结构损失联合删除实验。
EXPERIMENT_PRESETS = {
    "full": {
        "ssim": True, "tversky": True, "skeleton": True, "direction": True,
    },
    "no_ssim": {
        "ssim": False, "tversky": True, "skeleton": True, "direction": True,
    },
    "no_tversky": {
        "ssim": True, "tversky": False, "skeleton": True, "direction": True,
    },
    "no_skeleton": {
        "ssim": True, "tversky": True, "skeleton": False, "direction": True,
    },
    "no_direction": {
        "ssim": True, "tversky": True, "skeleton": True, "direction": False,
    },
    "no_skeleton_direction": {
        "ssim": True, "tversky": True, "skeleton": False, "direction": False,
    },
    "no_skeleton_gan": {
        "ssim": True, "tversky": True, "skeleton": False,
        "direction": True, "gan": False,
    },
}

# 正式模型采用消融实验中表现最优的配置：移除 Skeleton，保留 Direction。
FINAL_EXPERIMENT = "no_skeleton"
FINAL_CHECKPOINT = (
    CHECKPOINT_DIR / "ablations" / f"seed{SEED}_{FINAL_EXPERIMENT}" / "best.pt"
)
