...
class UnifiedGenerator(nn.Module):
    ...
    def __init__(self, in_ch=4, base_ch=64, num_downs=5, num_res=8):
        ...
        # ── 分支 B 瓶颈（Dilated ResBlock 修复）──
        self.bottleneck_B = nn.Sequential(
            ResnetBlock(ch, dilation=1),
            ResnetBlock(ch, dilation=2),
            ResnetBlock(ch, dilation=4),
            ResnetBlock(ch, dilation=8),
            ResnetBlock(ch, dilation=4),
            ResnetBlock(ch, dilation=2),
            ResnetBlock(ch, dilation=1),
            ResnetBlock(ch, dilation=1),
